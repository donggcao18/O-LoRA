# coding=utf-8
"""Task metadata and instruction pools for code tasks.

This mirrors the user's original `task_info.py` used by their dataloader.
It is imported by `code_tasks_dataset.py` to:
- pick an instruction template from INSTRUCTION_POOL per task_type
- do split-aware template sampling (train/val use head of pool; test uses full pool)

Note: Keep this file lightweight and free of heavy ML dependencies.
"""
